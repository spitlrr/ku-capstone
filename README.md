# ku-capstone
Capstone is a senior level course designed to allow a student to review, analyze, integrate, and apply technical knowledge in a meaningful and practical manner. The capstone course will be used to integrate concepts from IT curriculum coursework as well as professional and managerial coursework into a project that allows students to put skills into action.

# ku-capstone
Capstone is a senior level course designed to allow a student to review, analyze, integrate, and apply technical knowledge in a meaningful and practical manner. The capstone course will >

## project structure
TBD

Main*
-feature/baseline-setup

## getting started
1. fork main repo on GitHub

2. clone fork to your local environment set up
```bash
git clone https://github/yourusername/ku-capstone
```
3. change to project directory
```cd /ku-capstone
```

## how to contribute

1.


-

